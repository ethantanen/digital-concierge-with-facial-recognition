...
app.post("/adduser", async (req, res) => {
  ...
  try {
    ...
    res.send({
      audio: await talk(user.name + "added to the system.")
    })
  } catch (err) {
    console.log(err)
    res.send({
      audio: await talk('I was unable to add you to the system.')
    })
  }
})
...
app.post('/conversation', async (req, res) => {
  ...
  lexRes = await lex.postContent('1234567', text)
